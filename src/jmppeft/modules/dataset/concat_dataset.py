...
class _MTConcatDataset(ConcatDataset[BaseData], Generic[TDataset]):
    ...
    datasets: list[Dataset[BaseData]]
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(cast(list[Dataset[BaseData]], datasets))
    ...
    def atoms_metadata(self) -> np.ndarray:
        return np.concatenate([cast(TDataset, d).atoms_metadata for d in self.datasets])
