...
from ...tasks.finetune import QM9Config, QM9Model
...
def jmp_l_qm9_config(
    target: QM9Target,
    base_path: Path,
    ckpt_path: Path,
):
    with jmp_l_ft_config_builder(QM9Config, ckpt_path) as (builder, config):
        # Optimizer settings
        ...
        return builder(config), QM9Model
