...
class Rank2Block(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_layers):
            if self.mixing_coordinates:
                if i < num_layers - 1:
                    self.stress_MLP.append(nn.Linear(emb_size * 9, emb_size * 9))
    ...
    def forward(self, edge_distance_vec, x_edge, edge_index, data):
        ...
        edge_outer = x_edge[:, :, None] * outer_product_edge.view(-1, 9)[:, None, :]
        ...
        if self.edge_level:
            ...
            if self.mixing_coordinates:
                edge_outer = edge_outer.view(nEdges, -1)  # (nEdges, 9 * emb_size_edge)
...
class Rank2DecompositionBlock(nn.Module):
    ...
    def forward(self, x_pointwise, sphere_points, data):
        ...
        if self.extensive:
            scalar = scatter(node_scalar.view(-1), data.batch, dim=0, reduce="sum")
        else:
            scalar = scatter(node_scalar.view(-1), data.batch, dim=0, reduce="mean")
        ...
        node_irrep2 = node_irrep2 * sphere_irrep2.view(1, num_sphere_samples, 5)
        node_irrep2 = torch.sum(node_irrep2, dim=1) / num_sphere_samples
        if self.extensive:
            irrep2 = scatter(node_irrep2.view(-1, 5), data.batch, dim=0, reduce="sum")
        else:
            irrep2 = scatter(node_irrep2.view(-1, 5), data.batch, dim=0, reduce="mean")

        # Change of basis to compute a rank 2 symmetric tensor
        vector = torch.zeros((len(data.natoms), 3), device=x_pointwise.device).detach()
