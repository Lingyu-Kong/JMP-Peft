...
def mask_src_nodes(edge_idx, mask_idx):
    if mask_idx is None:
        return edge_idx
    src, tgt = edge_idx
    ...
    for t in mask_idx:
        masked[src == t] = 1
    masked = masked.bool()
    return torch.stack([src[~masked], tgt[~masked]], dim=0)


def mask_edges(edge_idx, mask_idx):
    if mask_idx is None:
        return edge_idx
    src, tgt = edge_idx
    masked = torch.zeros_like(src)
    for p, q in mask_idx:
        masked[(src == p) & (tgt == q)] = 1
    masked = masked.bool()
    return torch.stack([src[~masked], tgt[~masked]], dim=0)
...
class Distance(nn.Module):
    ...
    def forward(self, pos, batch, src_mask=None):
        ...
        if src_mask:
            edge_index = mask_src_nodes(edge_index, src_mask)
        ...
        # edge_index = mask_src_nodes(edge_index, src_mask_idx)
        ...
        if self.loop:
            ...
            mask = edge_index[0] != edge_index[1]
            edge_weight = torch.zeros(edge_vec.size(0), device=edge_vec.device).float()
            edge_weight[mask] = torch.norm(edge_vec[mask], dim=-1).float()
        else:
            edge_weight = torch.norm(edge_vec, dim=-1).float()
...
def _l2_normalize(d):
    ...
    d /= norm.repeat(*(1 for _ in range(d.dim() - 1)), d.shape[-1]) + 1e-8
    return d


def random_noise(ref, alpha, scale_by_norm=False, adjust_to_center=True):
    d = torch.rand(ref.shape).sub(0.5).to(ref.device)
    d = _l2_normalize(d)
    d = alpha * d
    if scale_by_norm:
        if adjust_to_center:
            norm = torch.norm(ref - ref.mean(0, keepdim=True), dim=-1, keepdim=True)
        else:
            norm = torch.norm(ref, dim=-1, keepdim=True)
        d *= norm.repeat(*(1 for _ in range(ref.dim() - 1)), ref.shape[-1])
    return d
