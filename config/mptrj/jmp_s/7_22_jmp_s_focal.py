...
freq_ratios_path = Path("/workspaces/repositories/jmp-peft/notebooks/freq_ratios.npy")


ckpt_dir = Path("/mnt/shared/checkpoints/")
jmp_s_ckpt_path = ckpt_dir / "jmp-s.pt"
jmp_l_ckpt_path = ckpt_dir / "jmp-l.pt"
...
def output_heads_config_(
    ...
    *,
    ...
    energy_loss = loss.MACEHuberEnergyFocalLossConfig(
        delta=0.01,
        freq_ratios_path=freq_ratios_path,
    )
    ...
    config.graph_targets.append(
        output_head.DirectStressTargetConfig(
            ...
            loss=loss.HuberStressFocalLossConfig(
                delta=0.01,
                freq_ratios_path=freq_ratios_path,
            ),
            reduction="mean",
    ...
    config.node_targets.append(
        output_head.NodeVectorTargetConfig(
            ...
            loss=loss.MACEHuberForceFocalLossConfig(
                delta=0.01,
                freq_ratios_path=freq_ratios_path,
            ),
            reduction="sum",
...
config.name_parts.append("focal")
