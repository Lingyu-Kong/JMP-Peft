...
df_in = pd.read_json(DATA_FILES.wbm_initial_structures).set_index(Key.mat_id)
...
df_in = pd.read_json(DATA_FILES.wbm_initial_structures).set_index(Key.mat_id)
structures = df_in[Key.init_struct].map(Structure.from_dict).to_dict()
...
df_entries = pd.read_json(DATA_FILES.wbm_cses_plus_init_structs)
